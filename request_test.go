...
func TestPostJSONBytesIoReader(t *testing.T) {
	...
	bodyBytes := unsafeConvert.ByteSlice(`{"username":"testuser", "password":"testpass"}`)
...
func TestPostXMLBytesSuccess(t *testing.T) {
	...
	resp, err := c.R().
		SetHeader(hdrContentTypeKey, "application/xml").
		SetBody(unsafeConvert.ByteSlice(`<?xml version="1.0" encoding="UTF-8"?><User><Username>testuser</Username><Password>testpass</Password></User>`)).
		SetQueryParam("request_no", strconv.FormatInt(time.Now().Unix(), 10)).
...
func TestMultiPartIoReaderFiles(t *testing.T) {
	...
	file := File{
		...
		Reader:    bytes.NewBuffer(unsafeConvert.ByteSlice("test bytes")),
	}
...
func TestMultiPartMultipartField(t *testing.T) {
	...
	jsonBytes := unsafeConvert.ByteSlice(`{"input": {"name": "Uploaded document", "_filename" : ["file.txt"]}}`)
